...
//        this.subjectService.update("CSES412", "Shabloni za dizajn na korisnicki interfejsi", 6, 5);
...
//    }
    ...
    void testUpdateActivity() {
        ...
        Activity activity = this.activityService.findById(1L).get();
    ...
    void testReportStudentAttendanceToActivity() {
        this.activityService.reportStudentAttendanceToActivity("181023", "tvXaH");
        Activity activity = this.activityService.findById(1L).get();

        assertEquals(activity.getNum_attendees().intValue(), 1);
        assertEquals(activity.getStudents().get(0).getIndex(), "181023");
    }

    @Test
    void testRemoveStudentFromAttendanceToActivity() {
        this.activityService.removeStudentFromAttendanceToActivity("181023", 1L);
        Activity activity = this.activityService.findById(1L).get();
    ...
    void testEnlistStudentToCourse() {
        this.studentService.enlistStudentToCourse("203045", 2L, "ivan.kitanovski");
        List<ListensTo> enlistings = this.studentListensToCourseRepository.findAll();

        assertEquals(enlistings.get(0).getStudent().getIndex(), "181023");
    ...
    void testActivitiesStatusCheckAndUpdate() {
        ...
        List<Activity> activities = this.activityService.findAll();
        assertEquals(activities.get(1).getTitle(), "Efikasen IT menadzment");
    }
}
